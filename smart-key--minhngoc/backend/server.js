require('dotenv').config(); //cho phép đọc các info của file .env
const express = require('express'); //tạo web server, rest API
const mysql = require('mysql2'); //dùng node.js kết nối và truy vấn db
const mqtt = require('mqtt'); //dùng để giao tiếp mqtt
const bcrypt = require('bcryptjs'); //dùng để hash và kiểm tra password
const cors = require('cors'); //cho phép fe và be giao tiếp với nhau dù khác port
const fs = require('fs'); //dùng để đọc các file chứng chỉ 
const path = require('path'); //dùng để tạo đường dẫn file


const app = express(); //khởi tạo để tạo ứng dụng express
app.use(express.json()); //cho phép server đọc db json từ request
app.use(cors()); //bật cors cho be

// ==========================================
//  CẤU HÌNH KẾT NỐI AWS RDS MYSQL
// ==========================================

//tạo connection pool để giữ một số connection để tái sử dụng 
//vì mỗi lần request lại sinh ra 1 connection
const db = mysql.createPool({
    host: process.env.DB_HOST, //địa chỉ server của MySQL
    user: process.env.DB_USER, //tên tài khoản MySQL
    password: process.env.DB_PASSWORD, //password của db
    database: process.env.DB_NAME,    //tên db 
    port: process.env.DB_PORT || 3306, //thiết lập port mặc định 
    waitForConnections: true,
    connectionLimit: 10 //giới hạn tối đa connection được tạo
});
...
//cấu hình mqtt ssl dùng để kiểm tra xác thực, bảo mật
//khi be kết nối với AWS IoT Core
const mqttOptions = {
...
//kiểm tra nếu message gửi đến topic đã subscribe thì hàm chạy
mqttClient.on('message', (topic, message) => {
    if (topic === 'MSSV/access_log') {
        try {
            const payload = JSON.parse(message.toString()); //message ban đầu là buffer => đổi thành obj js
            const cardUid = payload.uid; //lấy uid từ json
            const accessTime = payload.access_time || new Date().toISOString().slice(0, 19).replace('T', ' '); //nếu esp32 không gửi thời gian thì lấy tgian hiện tại
            ...
            db.query('SELECT status FROM DoorStatus ORDER BY id DESC LIMIT 1', (err, doorRows) => {
                ...
                db.query('SELECT * FROM CardInfo WHERE uid = ?', [cardUid], (err, cardRows) => {
                    ...
                    if (cardRows.length === 0) {
                        ...
                        mqttClient.publish('MSSV/door_control', JSON.stringify({mode: 'AUTO', action: 'OPEN' })); //be publish cho mqtt
                    }
                });
            });
        } catch (e) {
